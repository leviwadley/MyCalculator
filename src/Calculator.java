...
public class Calculator extends JFrame implements ActionListener  
{
	...
	private String tmpString = "7:8:9:+:4:5:6:-:1:2:3:*:.:/:C:\u221A:\u00B1:=:0:";
	private String[] btnString = tmpString.split(":");
	...
	{ 
		...
		for (int i = 0; i < 14; i++ )
		{
			if (i < 4){row[1].add(button[i]);}
			else if (i >= 4 && i < 8)row[2].add(button[i]);
			else if (i >= 8 && i < 12)row[3].add(button[i]);
			else if (i >= 12 && i <14 ){
				 if (i == 12 ) {row[4].add(button[18]);}
				row[4].add(button[i]); 
				}
		}
		row[1].add(button[14]);
